...
def routing_tool(state: State) -> Literal["retriever", "general_llm", "web_search", "team_navigator"]:
    """
    ...
    Returns:
        The next node to execute: "retriever", "general_llm", "web_search",
        or "team_navigator" for SME/people-finding queries.
    """
    if state["route"] == "index":
        return "retriever"
    elif state["route"] == "team_nav":
        return "team_navigator"
...
def rewrite_router(state: State) -> Literal["retriever", "team_navigator"]:
    """
    After rewriting a query, route back to the appropriate lookup node
    based on the original query route. Team navigation queries should
    return to the team navigator; document queries return to the retriever.

    Args:
        state (State): The current state of the graph.

    Returns:
        "team_navigator" if the route is team_nav, otherwise "retriever".
    """
    if state.get("route") == "team_nav":
        return "team_navigator"
    return "retriever"
