...
def get_master_llm():
    ...
    Note: Local Ollama fallback is commented out to prevent CPU hangs on local Windows dev.
    It is left here as documentation of the production on-premise fallback architecture.
    """
    ...
    if mistral_key:
        try:
            ...
            return ChatMistralAI(
                ...
                temperature=0.7,
                max_retries=0
                # REMOVED request_timeout=30 because Mistral API rejects it
            )
        except Exception as e:
